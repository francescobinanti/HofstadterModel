...
parser.add_argument('--absorptionshift', type=int, default=0, help='to be specified if the absorption spectrum we want to plot has been generated by acting with the laser upon a state different from the ground state (i.e. 0)')
...
# Pfaffian preparation
parser.add_argument('--maxoverlapinteraction', type=int, nargs='?', const=1, default=0, help='to plot the maximum overlaps with respect to the variation of interaction U/U3 generated by StatesOverlap.py (specify the values of -U and -U3 used for the overlap)')
# U2,U3 fidelity
parser.add_argument('--ufidelity', type=int, nargs='?', const=1, default=0, help='to plot the fidelity F=|<psi_n1(U,U3)|psi_n1(U-du,U3+dU)>| (need to specify -n1 that is the eigenstate label we are calculating the fidelity for)')
parser.add_argument('-n1', type=int, default=0, help='the eigenstate label to calculate the fidelity for (default=0, ground state)')
parser.add_argument('--ufidelityadaptive', type=int, nargs='?', const=1, default=0, help='to plot the fidelity F=|<psi_n(U,U3)|psi_np(U-du,U3+dU)>| (n,np are adaptive on the basis of the largest overlaps)')
# Plot all the overlaps with respect to the interaction U2,U3
parser.add_argument('--alloverlaps', type=int, nargs='?', const=1, default=0, help='flag to plot all the overlaps generated in the file _alloverlaps from StatesOverlap.py as a function of U2 or U3')
# adaptive correlation functions
parser.add_argument('--ucorrfuncadaptive', type=int, nargs='?', const=1, default=0, help='to plot the C2,C3 correlation functions adapted to the eigenstate of the largest overlap (if --ucorrorder is specified then it plots only one of the two)')
parser.add_argument('--ucorrorder', type=int, default=0, help='order of the correlation function to plot')
# Free plotting
parser.add_argument('--freeplotonecol', type=str, help='provide the filename of one column data to plot')
...
if args.L is not None: Ns = L*L
if args.L is not None: c0 = gm.FindCenter(L)
...
if args.freeplotonecol is not None:
    fileName = args.freeplotonecol
    yData = gm.LoadFileOne(fileName)
    
    xlabel = args.xlabel
    ylabel = args.ylabel
    plt.xlabel(f'{xlabel}')
    plt.ylabel(f'{ylabel}')
    plt.xticks(np.arange(0,len(yData),5))
    if args.sety != 0:
        plt.ylim(top=args.sety)
        
    plt.scatter(np.arange(0,len(yData)), yData, marker='o', s=3.0, facecolors='#0066cc')
...
if args.absorption == 1:
    ...
    shift = args.absorptionshift
    
    # l < 0 region
    fileName = gm.GenFilename(hardcore, L, J, U, trapConf, gamma, shift, U3=U3, alpha=FluxDensity, N=N, absSpectrum=True, r0=args.r0)
...
if args.ucorrfuncadaptive == 1:
    
    plt.xlabel(r'$U_3$')
    plt.ylabel(r'$C(n_{largest overlap})$')
    if (args.ucorrorder == 2) or (args.ucorrorder == 0):
        fileName = gm.GenFilename(hardcore, L, J, U, trapConf, gamma, args.n1, U3=U3, alpha=FluxDensity, N=N, corrFunction=2) + '_adaptive'
        U3Vals, cFunction = gm.LoadFileTwo(fileName)
        plt.scatter(U3Vals, cFunction, marker='o', s=1.5, facecolors='orange', label='2-point')
        plt.plot(U3Vals, cFunction, color='orange', linewidth=0.7)
    if (args.ucorrorder == 3) or (args.ucorrorder == 0):
        fileName = gm.GenFilename(hardcore, L, J, U, trapConf, gamma, args.n1, U3=U3, alpha=FluxDensity, N=N, corrFunction=3) + '_adaptive'
        U3Vals, cFunction = gm.LoadFileTwo(fileName)
        plt.scatter(U3Vals, cFunction, marker='o', s=1.5, facecolors='#0066cc', label='3-point')
        plt.plot(U3Vals, cFunction, color='#0066cc', linewidth=0.7)
        plt.legend()
...
if args.maxoverlapinteraction == 1:
    fileName = gm.GenFilename(hardcore, L, J, 0.0, trapConf, gamma, 0, U3=round(U3,6), alpha=FluxDensity, N=N, maxOverlap=True) # Pfaffian overlap <psi_Pf|...>
    U3vals, nMaxOverlap, overlap = gm.LoadFileThree(fileName)
    
    plt.xlabel(r'$U_3/U_2$')
    plt.ylabel(r'$n_{eigen}$')
    plt.yticks(np.arange(0,65,6))
    plt.scatter(U3vals, nMaxOverlap, marker='o', s=3.5, facecolors='#0066cc', label='Pfaffian')
    plt.plot(U3vals, nMaxOverlap, color='#0066cc', linewidth=0.7)
    # Write the overlap values near the datapoints
    for i in np.arange(0,len(overlap)):
        plt.text(U3vals[i] + 0.05, nMaxOverlap[i] + 0.1, round(overlap[i],2), fontsize=10, color='black')
    
    fileName = gm.GenFilename(hardcore, L, J, round(U,6), trapConf, gamma, 0, U3=0.0, alpha=FluxDensity, N=N, maxOverlap=True) # Laughlin overlap <psi_Pf|...>
    U3vals, nMaxOverlap, overlap = gm.LoadFileThree(fileName)
    
    plt.scatter(U3vals, nMaxOverlap, marker='o', s=3.5, facecolors='orange', label='Laughlin')
    plt.plot(U3vals, nMaxOverlap, color='orange', linewidth=0.7)
    for i in np.arange(0,len(overlap)):
        plt.text(U3vals[i], nMaxOverlap[i] + 1.0, round(overlap[i],2), fontsize=5, color='black')
    
    plt.legend()
    plt.savefig(fileName+'.pdf')
    
if args.ufidelity == 1:
    fileName = gm.GenFilename(hardcore, L, J, 0, trapConf, gamma, args.n1, U3=0, alpha=FluxDensity, N=N) + '_ufidelity'
    U3vals, fidelity = gm.LoadFileTwo(fileName)

    plt.xlabel(r'$U_3$')
    plt.ylabel(f'$F_{{{args.n1}}}$')
    plt.xticks(np.arange(U3vals[0],U3vals[-1],1.5))
    plt.scatter(U3vals, fidelity, marker='o', s=3.5, facecolors='#0066cc')
    plt.plot(U3vals, fidelity, color='#0066cc', linewidth=0.7)
    
    plt.savefig(fileName+'.pdf')
    
if args.ufidelityadaptive == 1:
    fileName = gm.GenFilename(hardcore, L, J, U, trapConf, gamma, args.n1, U3=U3, alpha=FluxDensity, N=N) + '_ufidelityadaptive'
    U3vals, fidelity = gm.LoadFileTwo(fileName)

    plt.xlabel(r'$U_3$')
    plt.ylabel(f'$F_{n1,n2}$')
    plt.xticks(np.arange(U3vals[0],U3vals[-1],1.5))
    plt.scatter(U3vals, fidelity, marker='o', s=3.5, facecolors='#0066cc')
    plt.plot(U3vals, fidelity, color='#0066cc', linewidth=0.7)
...
if args.alloverlaps == 1:
    fileName = gm.GenFilename(hardcore, L, J, U, trapConf, gamma, args.n1, U3=U3, alpha=FluxDensity, N=N) + '_alloverlaps'
    U3Vals, overlaps = gm.LoadFileTwo(fileName)
    
    # overlaps > 0.5
    boolTable = overlaps > 0.5
    overlapsAbove = overlaps[boolTable]
    U3Above = U3Vals[boolTable]
    
    # overlaps < 0.5
    boolTable = overlaps < 0.5
    overlapsBelow = overlaps[boolTable]
    U3Below = U3Vals[boolTable]
    
    if args.sety != 0:
        plt.ylim(top=args.sety)
    
    plt.xlabel(r'$U_3$')
    plt.ylabel(f'$<\psi(U_3={{{U3}}})|\psi_n>$')
    plt.xticks(np.arange(0,U3Vals[-1],1.5))
    plt.scatter(U3Above, overlapsAbove, marker='o', s=3.0, facecolors='red', label=f'$>0.5$')
    plt.scatter(U3Below, overlapsBelow, marker='o', s=3.0, facecolors='#0066cc', label=f'$<0.5$')
