...
if excFrac == True:
    legend = []
    colors = ['#0066cc', 'g', 'orange']
    c = 0

    for w in np.arange(wInit, wFinal, wStep):
        fileName = gm.GenFilename(hardcore, L, J, U, trapConf, gamma, 0, U3=U3, alpha=FluxDensity, N=N, excFrac=True, timeEvolAngMom=angMom, timeEvolEps=eps, timeEvolOmega=round(w,2))
        t, excFracData = gm.LoadFileTwo(fileName)
        
        plt.xlabel(r'$t$')
        plt.ylabel(r'$N_{l,\omega}(t)$')
        plt.scatter(t, excFracData, marker='o', s=1.5, facecolors=colors[c%3])
